...
class MainController extends BaseController
{
        ...
        try {
            ...
            if ($statusCode == 200) {
                ...
                $request->session()->put('user_email', $data['user']['email']);
                 DB::table('bigcommerce_app_installs')->where('store_hash', $request->input('context'))->delete();
                 DB::table('bigcommerce_app_installs')->insert([
    ...
    public function load(Request $request)
    {
        ...
        return redirect()->route('orders', $verifiedSignedRequestData);
    }
    ...
    public function makeBigCommerceAPIRequest(Request $request, $endpoint = "v2/orders")
    {

        $requestConfig = [
            'headers' => [
                'X-Auth-Client' => 'p24aiip2zs7f1in3r90j7pzfbw0ftb',
                'X-Auth-Token'  => '5s76yonocqnckhf7eo89z8a1jstld0e',
        ...
        $client = new Client(['verify'=>false]);
        $result = $client->request('GET', 'https://api.bigcommerce.com/stores/38z39l51z5/' . $endpoint, $requestConfig);
        return $result;
    }
